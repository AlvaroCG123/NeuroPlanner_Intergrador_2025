        ...
        <section className="bg-[#0d1a23]/80 rounded-xl p-8 md:p-12 flex flex-col lg:grid lg:grid-cols-[1fr_auto_1fr] lg:items-center gap-8">
          <div className="w-full lg:col-span-1">
            <div className="space-y-6">
              <div>
                <label className="block text-sm md:text-base text-white/80 mb-2 uppercase">Nome</label>
                <input value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Nome completo" className="w-full bg-[#13232c] placeholder-gray-400 text-white p-3 rounded-lg text-sm md:text-base" />
              </div>

              <div>
                <label className="block text-sm md:text-base text-white/80 mb-2 uppercase">Nascimento</label>
                <input value={birth} onChange={(e) => setBirth(e.target.value)} type="date" className="w-full bg-[#13232c] placeholder-gray-400 text-white p-3 rounded-lg text-sm md:text-base" />
              </div>
          ...
          <div className="w-full lg:col-span-1">
            <div className="space-y-6">
              <div>
                <label className="block text-sm md:text-base text-white/80 mb-2 uppercase">Condição</label>
                <input value={condition} onChange={(e) => setCondition(e.target.value)} type="text" placeholder="(ex.: TDAH, Autismo...)" className="w-full bg-[#13232c] placeholder-gray-400 text-white p-3 rounded-lg text-sm md:text-base" />
              </div>

              <div>
                <label className="block text-sm md:text-base text-white/80 mb-2 uppercase">Medicamentos em uso</label>
                <input value={meds} onChange={(e) => setMeds(e.target.value)} type="text" placeholder="Lista de medicamentos" className="w-full bg-[#13232c] placeholder-gray-400 text-white p-3 rounded-lg text-sm md:text-base" />
              </div>

              <div>
                <label className="block text-sm md:text-base text-white/80 mb-2 uppercase">Informações importantes</label>
                <p className="text-[11px] text-white/60 mb-2">(Alergias, gatilhos, preferências de comunicação, etc.)</p>
                <textarea value={info} onChange={(e) => setInfo(e.target.value)} rows={4} placeholder="Anotações importantes" className="w-full bg-[#13232c] placeholder-gray-400 text-white p-3 rounded-lg resize-none text-sm md:text-base"></textarea>
              </div>

              <div className="flex justify-end">
                <button onClick={handleSave} className="px-4 py-2 bg-[#4F77F1] rounded-md">Salvar</button>
              </div>
            </div>
          </div>
        </section>
